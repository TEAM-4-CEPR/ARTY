...
Change from 0.0.1
@ This script no longer use PydesEq2 (cause I can't fucking export the DE matrix , and tbh tf is a pickle format????) , 
now use DESEQ2 from R and pass all args to R. 
...
def pca3d():
	...
	print(components)
	fig = px.scatter_3d(components, x=0, y=1, z=2, color=design["sample"].to_list(),
...
def differential_analysis():
	...
	count matrix. It also creates the design matrix
	"""
...
'Non_significatively_up_regulated', 'non_significatively_down_regulated']
	...
	conditions = [
		...
		(df['log2FoldChange'] < 0) & (df['padj'] > 0.05)]
	values = [
		'Significatively_down_regulated', 'Significatively_up_regulated',
		'Non_significatively_up_regulated', 'non_significatively_down_regulated']
...
fenetre = Tk()
fenetre.title('Analyse youR daTa Yourself')
fenetre.geometry("960x540")
fenetre.configure(bg='black')
bgimg = PhotoImage(file="./image/interface.png")
limg = Label(fenetre, i=bgimg)
limg.place(x=0, y=0)
window_label = Label(fenetre, text="ARTY")
window_label.pack()
...
path_t.pack()
sample_data_image = PhotoImage(file="./image/sample_data.png")
...
target_t.pack()
design_matrix_image = PhotoImage(file="./image/design.png")
...
entree.pack()
"""
pca_2d_image = PhotoImage(file="./image/pca2d.png")
pca_3d_image = PhotoImage(file="./image/pca3d.png")
go_image = PhotoImage(file="./image/Go.png")
heatmap_image = PhotoImage(file="./image/heatmap.png")
de_image = PhotoImage(file="./image/De.png")
volcano_image = PhotoImage(file="./image/volcano.png")
